...
btnCalculateNumOfDay.onclick = function () {
  ...
  if (validateDate(2, month, year)) {
    ...
    txtNumberOfDay.innerHTML =
      "Tháng " + month + "/" + year + " có:" + result + " ngày";
  } else {
...
btnReadNum.onclick = function () {
  ...
  if (inputNum.toString().length !== 3 && inputNum > 0) {
    txtReadResult.innerHTML = " Số không hợp lệ";
    return;
  }
  if (inputNum.toString().length !== 4 && inputNum < 0) {
    txtReadResult.innerHTML = " Số không hợp lệ";
    return;
  }
  if (inputNum > 0) {
    ...
    if (unitNum == 0 && dozenNum == 0) {
      ...
      result =
        readHundredNum(leftNum) +
        " trăm " +
        readDozenNum(dozenNum) +
    ...
    if (unitNum == 0 && dozenNum == 0) {
      result = "âm " + readHundredNum(leftNum) + " trăm ";
    } else {
      result =
        "âm " +
        readHundredNum(leftNum) +
